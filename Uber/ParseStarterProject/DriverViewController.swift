...
class DriverViewController: UITableViewController, CLLocationManagerDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        else if segue.identifier == "showRiderLocationViewController"
        {
           if let destination = segue.destination as? RiderLocationViewController
           {
                ...
                {
                destination.riderLocation = (requestLocations[row])
                destination.riderUsernameLocation = (requestsUserNames[row])
                }
           }
        }
    }
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let location = manager.location?.coordinate
        {
            ...
            query.findObjectsInBackground(block: { (objects, error) in
                ...
                {
                    self.requestsUserNames.removeAll()
                    ...
                    for riderRequest in riderRequests
                    {
                        if let username = riderRequest["username"] as? String
                        {
                            ...
                            {
                            self.requestsUserNames.append(username)
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return requestsUserNames.count
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Find the distance between userLocation and requestLocation[indexPath.row]
        ...
        let distance = String(format:"%.2f" ,driverCLLocation.distance(from: riderCLLocation) / 1000)
        
        //String(format: "%.2f", showgetedDistances[0] * 0.001)
        
        cell.textLabel?.text = requestsUserNames[indexPath.row] + " - \(distance) km away"
